...
type TransformFekmNewsResponse =
  | {
      ok: true;
      data: {
        titulo: string;
...
function jsonWithCors(
  request: Request,
  payload: TransformFekmNewsResponse,
  init?: ResponseInit
): NextResponse {
...
function createContextPrompt(params: {
  ...
  const fekmOrganization = sanityContext.organizaciones.find((organization) =>
    normalizeText(getDocLabel(organization)).includes("fekm")
  );

  const candidateDisciplines = sanityContext.disciplinas.filter((discipline) => {
    const label = getDocLabel(discipline);
    return ["Kickboxing", "Muay Thai"]
  ...
  const organizationLabels = compactList(
    [
      fekmOrganization ? getDocLabel(fekmOrganization) : "FEKM",
  ...
  return [
    ...
    `Organizaciones candidatas: ${organizationLabels.join(" | ") || "FEKM"}`,
    "",
...
function validateRelations(
  ...
  return {
    ...
    organizacion: getOutputString(relations, "organizacion") || "FEKM",
  ...
  try {
    ...
    if (!apiKey) {
      return jsonWithCors(
        ...
        {
          ok: false,
          error: "Falta OPENAI_API_KEY para transformar noticias FEKM.",
        },
        { status: 500 }
      );
    }

    let body: TransformFekmNewsBody;

    try {
      body = (await request.json()) as TransformFekmNewsBody;
    } catch {
      return jsonWithCors(
        request,
        {
    ...
    if (!title) {
      return jsonWithCors(
        ...
        {
          ok: false,
          error: "Falta el título de la noticia FEKM.",
        },
        { status: 400 }
      );
    }
    ...
    if (sourceText.length < 80) {
      return jsonWithCors(
        ...
        {
          ok: false,
          error: "La noticia FEKM no tiene contenido suficiente para transformarse con seguridad.",
        },
        { status: 400 }
      );
    }
      ...
      messages: [
        {
          ...
          content: [
            ...
            "Tu prioridad es reescribir con voz editorial propia, entender la noticia completa, detectar el hecho principal y resolver relaciones editoriales útiles.",
            "FEKM cubre principalmente Kickboxing y Muay Thai; elige la disciplina real por el contexto y no uses MMA por defecto.",
            "No copies frases extensas ni traduzcas literalmente.",
            "Conserva hechos, nombres, fechas, resultados, citas y atribución a FEKM.",
          ...
          content: [
            ...
            JSON.stringify({
              ...
              relacionesSugeridas: {
                ...
                organizacion: "FEKM",
                disciplina:
                  "Disciplina principal: Kickboxing o Muay Thai según el contenido y las referencias existentes en Sanity",
              },
            }),
          ].join("\n"),
        },
      ...
      request,
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Error desconocido transformando noticia FEKM.",
      },
      { status: 500 }
    );
  }
