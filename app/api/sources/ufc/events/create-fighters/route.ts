...
async function fetchSanityContext(): Promise<{
  ...
  const [
    ...
    sanityClient.fetch<ReferenceDoc | null>(
      `*[_type == "disciplina" && lower(nombre) == "mma"][0]{
        _id,
        nombre
      }`,
      {},
    ...
    sanityClient.fetch<ReferenceDoc | null>(
      `*[_type == "organizacion" && lower(nombre) == "ufc"][0]{
        _id,
        nombre
      }`,
      {},
...
export async function POST(
  ...
  try {
        ...
        NextResponse.json(
          {
            ok: false,
            error:
              "No se encontró la disciplina MMA en Sanity.",
          },
          { status: 409 }
        ),
        request
        ...
        NextResponse.json(
          {
            ok: false,
            error:
              "No se encontró la organización UFC en Sanity.",
          },
          { status: 409 }
        ),
        request
    ...
    const mmaCategories = categories.filter(
      (category) =>
    ...
    for (const candidate of candidates) {
      ...
      const documentId = `ufc-fighter-${candidate.slug}`;
    ...
    console.error(
      "Error creando luchadores faltantes desde UFC:",
      error
    );
