...
async function fetchContext(): Promise<{
  ...
  const [events, fighters, categories, mma] = await Promise.all([
...
function buildFightDocumentId(params: {
  ...
  return `ufc-fight-${eventPart}-${redPart}-vs-${bluePart}`.slice(
    0,
    200
  );
}
...
export async function POST(
  ...
  try {
    ...
    const mmaCategories = mma
      ? categories.filter(
          (category) =>
            !category.disciplina?._ref ||
            baseId(category.disciplina._ref) === baseId(mma._id)
        )
      : categories;
    ...
    console.error(
      "Error creando combates UFC en Sanity:",
      error
    );
