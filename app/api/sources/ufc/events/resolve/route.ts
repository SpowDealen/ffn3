...
const CATEGORY_ALIASES: Record<string, string[]> = {
  ...
  "women's flyweight": ["peso mosca femenino", "mosca femenino", "peso mosca"],
  "bantamweight": ["peso gallo", "gallo"],
  "women's bantamweight": ["peso gallo femenino", "gallo femenino", "peso gallo"],
  "featherweight": ["peso pluma", "pluma"],
  "women's featherweight": ["peso pluma femenino", "pluma femenino", "peso pluma"],
  ...
  "women's strawweight": ["peso paja femenino", "paja femenino", "peso paja", "paja"],
  ...
  const [discipline, organization, events, fighters, categories] =
    await Promise.all([
      sanityClient.fetch<SanityReferenceDoc | null>(
        `*[_type == "disciplina" && lower(nombre) == "mma"][0]{_id,nombre,slug}`,
        {},
        { perspective: "raw" }
      ),
      sanityClient.fetch<SanityReferenceDoc | null>(
        `*[_type == "organizacion" && lower(nombre) == "ufc"][0]{_id,nombre,slug}`,
        {},
        { perspective: "raw" }
      ),
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const mmaCategories = discipline
      ? categories.filter(
          (category) =>
    ...
    const ufcFighters = fighters.filter((fighter) => {
    ...
    const resolvedFights: ResolvedFight[] = fightCard.map((fight) => {
      ...
      const category = resolveCategory(fight.weightClass, mmaCategories);

      const winnerName = getString(fight.winnerName);
      const winner = winnerName
        ? resolveReferenceByName(winnerName, ufcFighters)
        : undefined;
      ...
      if (!discipline) {
        blockingReasons.push("disciplina_mma_no_encontrada");
      }

      if (!organization) {
        blockingReasons.push("organizacion_ufc_no_encontrada");
      }
    ...
    console.error("Error resolviendo cartelera UFC contra Sanity:", error);
