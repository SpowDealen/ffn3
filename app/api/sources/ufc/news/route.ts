...
const UFC_NEWS_URL = `${UFC_BASE_URL}/trending/all`;
...
type UfcNewsApiResponse = {
  ok: boolean;
  source: "ufc";
  fetchedAt: string;
  count: number;
  items: UfcNewsApiItem[];
  error?: string;
};
...
function createAbsoluteUrl(value: string): string {
  ...
  if (trimmedValue.startsWith("/")) {
    return `${UFC_BASE_URL}${trimmedValue}`;
  }
...
function createItemId(sourceUrl: string): string {
  try {
    ...
    return `ufc-news-${normalizedValue}`;
  }
}
  ...
  try {
    ...
    const items = articleResults
      .filter(
        (
          item,
        ): item is UfcNewsApiItem =>
          item !== null,
      )
      .map((item) => sanitizeArticle(item));

    const payload: UfcNewsApiResponse = {
      ok: true,
      source: "ufc",
      fetchedAt,
      count: items.length,
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Error desconocido consultando UFC.";
