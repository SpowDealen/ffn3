...
function hasEnoughContent(item: FekmOfficialNewsItem): boolean {
  const sourceText =
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    if (items.length === 0) {
      ...
      const newsResponse = await fetch(`${origin}/api/sources/fekm/news`, {
        method: "GET",
        cache: "no-store",
      });
        ...
        return NextResponse.json(
          {
            ...
            error:
              newsData.error ||
              "No se pudieron cargar noticias FEKM Championship para analizarlas.",
          },
          {
            status: 500,
            headers: {
    ...
    if (items.length === 0) {
      return NextResponse.json(
        {
          ok: false,
          error: "No se encontraron noticias FEKM Championship válidas para analizar.",
        },
        {
          status: 400,
          headers: {
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Error desconocido analizando noticias FEKM Championship.",
      },
      {
        status: 500,
        headers: {
