...
const BKFC_NEWS_URL = `${BKFC_BASE_URL}/news`;
...
type BkfcNewsApiResponse = {
  ok: boolean;
  source: "bkfc";
  fetchedAt: string;
  count: number;
  items: BkfcNewsApiItem[];
  error?: string;
};
...
function createAbsoluteUrl(value: string): string {
  ...
  if (trimmedValue.startsWith("/")) {
    return `${BKFC_BASE_URL}${trimmedValue}`;
  }
...
function createItemId(sourceUrl: string): string {
  try {
    ...
    return `bkfc-news-${normalizedValue}`;
  }
}
...
async function fetchHtml(url: string): Promise<string> {
  const response = await fetch(url, {
    headers: {
      ...
      "User-Agent":
        "Mozilla/5.0 (compatible; FullFightNewsBkfcSourceReader/1.0)",
    },
    cache: "no-store",
  });
  ...
  try {
    ...
    const items = articleResults
      .filter(
        (
          item,
        ): item is BkfcNewsApiItem =>
          item !== null,
      )
      .map((item) => sanitizeArticle(item));

    const payload: BkfcNewsApiResponse = {
      ok: true,
      source: "bkfc",
      fetchedAt,
      count: items.length,
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Error desconocido consultando BKFC.";
