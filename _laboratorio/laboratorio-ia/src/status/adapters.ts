...
export function adaptRuntimeStatus(observation: RuntimeObservation): SubsystemStatus {
  if (observation.state === "checking") return status({id: "runtime", label: "Runtime", state: "recovering", effect: "recovering", summary: "Comprobando rutas seguras del laboratorio.", route: "/editorial", activeCount: 0, currentIncidentCount: 0, historicalCount: 0, isLive: true});
  if (observation.state === "unavailable") return status({id: "runtime", label: "Runtime", state: "unavailable", effect: "unavailable", summary: "Las rutas del laboratorio no responden.", reason: observation.reason ?? "runtime_unreachable", route: "/editorial", checkedAt: observation.checkedAt, activeCount: 0, currentIncidentCount: 1, historicalCount: 0, isLive: true});
  return status({id: "runtime", label: "Runtime", state: "operational", effect: "none", summary: "Runtime accesible desde una ruta segura.", route: "/editorial", checkedAt: observation.checkedAt, activeCount: 0, currentIncidentCount: 0, historicalCount: 0, isLive: true});
}

export function adaptReferenceEntitiesStatus(observation: ReferenceEntitiesObservation): SubsystemStatus {
  if (observation.state === "checking") return status({id: "references", label: "Reference Entities", state: "recovering", effect: "recovering", summary: "Comprobando lectura editorial.", route: "/editorial", activeCount: 0, currentIncidentCount: 0, historicalCount: 0, isLive: true});
  if (observation.state === "unavailable") return status({id: "references", label: "Reference Entities", state: "blocked", effect: "blocked", summary: "La referencia editorial transversal no está disponible.", reason: observation.reason ?? "reference_entities_unavailable", route: "/editorial", checkedAt: observation.checkedAt, activeCount: 0, currentIncidentCount: 1, historicalCount: 0, isLive: true});
  return status({id: "references", label: "Reference Entities", state: "operational", effect: "none", summary: `${observation.entityCount ?? 0} referencias disponibles.`, route: "/editorial", checkedAt: observation.checkedAt, activeCount: 0, currentIncidentCount: 0, historicalCount: 0, isLive: true});
}
...
export function adaptTelegramStatus(input: Readonly<{checking: boolean; health: TelegramHealthResponse | null; error?: string}>): SubsystemStatus {
  if (input.checking) return status({id: "telegram", label: "Telegram", state: "recovering", effect: "recovering", summary: "Comprobando health vivo.", route: "/telegram", activeCount: 0, currentIncidentCount: 0, historicalCount: 0, isLive: true});
  ...
  if (health.deliveryMode === "sandbox") return status({id: "telegram", label: "Telegram", state: "operational", effect: "none", summary: "Sandbox seguro; dispatch externo bloqueado.", route: "/telegram", checkedAt: health.checkedAt, activeCount: 0, currentIncidentCount: 0, historicalCount: 0, isLive: true});
  ...
  return status({id: "telegram", label: "Telegram", state: "operational", effect: "none", summary: "Health vivo disponible.", route: "/telegram", checkedAt: health.checkedAt, activeCount: 0, currentIncidentCount: 0, historicalCount: 0, isLive: true});
}
...
export function adaptNotificationsStatus(notifications: readonly LabNotification[]): SubsystemStatus {
  ...
  return status({
    id: "notifications", label: "Notifications", state: pending ? "active" : "operational", effect: pending ? "active" : "none",
...
export function adaptProcessesStatus(processes: readonly ProcessExperiencePresentation[]): SubsystemStatus {
  ...
  if (currentBlocked.length) return status({id: "processes", label: "Processes", state: "blocked", effect: infrastructureBlocked ? "degraded" : "attention", summary: `${currentBlocked.length} procesos bloqueados.`, reason: infrastructureBlocked ? "process_infrastructure_blocked" : "process_domain_blocked", route: "/actividad", activeCount: active, currentIncidentCount: currentBlocked.length, historicalCount: processes.filter((process) => process.isHistorical).length, isLive: active > 0});
  if (active) return status({id: "processes", label: "Processes", state: "active", effect: "active", summary: `${active} procesos en ejecución.`, route: "/actividad", activeCount: active, currentIncidentCount: 0, historicalCount: processes.filter((process) => process.isHistorical).length, isLive: true});
  return status({id: "processes", label: "Processes", state: "idle", effect: "none", summary: "Sin procesos activos.", route: "/actividad", activeCount: 0, currentIncidentCount: 0, historicalCount: processes.filter((process) => process.isHistorical).length, isLive: false});
}
...
export function adaptReviewStatus(cases: readonly ReviewCase[]): SubsystemStatus {
  ...
  if (blocked.length) return status({id: "review", label: "Review / AU7 / AU8", state: "blocked", effect: "degraded", summary: `${blocked.length} casos requieren desbloqueo o reconciliación.`, reason: "review_capability_blocked", route: "/revision", activeCount: active.length, currentIncidentCount: blocked.length, historicalCount: cases.length - current.length, isLive: active.length > 0});
  if (active.length) return status({id: "review", label: "Review / AU7 / AU8", state: "active", effect: "active", summary: `${active.length} operaciones supervisadas en curso.`, route: "/revision", activeCount: active.length, currentIncidentCount: 0, historicalCount: cases.length - current.length, isLive: true});
  if (current.length) return status({id: "review", label: "Review / AU7 / AU8", state: "attention", effect: "attention", summary: `${current.length} casos esperan intervención.`, reason: "review_attention_required", route: "/revision", activeCount: 0, currentIncidentCount: 0, historicalCount: cases.length - current.length, isLive: true});
  return status({id: "review", label: "Review / AU7 / AU8", state: "idle", effect: "none", summary: "Sin casos operativos pendientes.", route: "/revision", activeCount: 0, currentIncidentCount: 0, historicalCount: cases.length, isLive: false});
}
