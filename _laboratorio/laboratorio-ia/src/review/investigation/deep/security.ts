...
export function assertSafeSerializable(value: unknown, maxBytes = 200_000, maxStringLength = 20_000): asserts value is ReviewJsonValue {
  const seen = new WeakSet<object>();
  const walk = (item: unknown, depth: number, key = ""): void => {
    if (SENSITIVE.test(key)) throw new Error("sensitive_key_rejected");
    if (depth > 12) throw new Error("maximum_depth_exceeded");
