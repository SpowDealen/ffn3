...
export function validateGlobalResolutionPlan(value: unknown): GlobalResolutionPlanValidationResult {
  ...
  if (full.operations.length && !resumeNodes.length && full.originalOperation !== "request_fighter_resolution" && !full.blockers.some((blocker) => blocker.scope === "structure" && blocker.severity === "blocking")) errors.push(issue("global_resolution_plan_resume_missing", "Un plan estructuralmente preparado necesita una reanudación explícita."));
