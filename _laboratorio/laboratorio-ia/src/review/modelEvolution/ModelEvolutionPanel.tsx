              ...
              return (
                <section className={recommended ? "model-evolution-alternative model-evolution-recommended" : "model-evolution-alternative"} key={alternative.id}>
                  ...
                  <div className="model-evolution-metrics" aria-label={`Estimación de ${alternative.title}`}>
                    <span><strong>{alternative.cost.totalHours.min}–{alternative.cost.totalHours.max} h</strong> coste provisional</span>
                    ...
                    <span><strong>{alternative.risk.editorialRisk}</strong> riesgo editorial de no actuar</span>
                  ...
                  <details open={recommended}>
                    ...
                    <ul>
                      <li>Auditoría: {alternative.cost.auditHours.min}–{alternative.cost.auditHours.max} h</li>
                      <li>Implementación: {alternative.cost.implementationHours.min}–{alternative.cost.implementationHours.max} h</li>
                      <li>Migración: {alternative.cost.migrationHours.min}–{alternative.cost.migrationHours.max} h</li>
                      <li>Validación: {alternative.cost.validationHours.min}–{alternative.cost.validationHours.max} h</li>
                    </ul>
                    ...
                    <h6>Migración y rollback</h6><p>{alternative.migration.strategy}</p><p><strong>{alternative.rollbackPossible ? "Rollback posible" : "Rollback no garantizado"}:</strong> {alternative.rollbackPlan}</p>
