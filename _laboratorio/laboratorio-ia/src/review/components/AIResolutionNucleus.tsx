...
export default function AIResolutionNucleus({reviewCase, canEdit, canFinalize, onFinalize, onContextChange}: {reviewCase: ReviewCase; canEdit: boolean; canFinalize: boolean; onFinalize(): void; onContextChange?(context: NucleusContextView): void}): ReactElement {
  ...
  return <section className="review-subsection ai-resolution-nucleus" aria-labelledby={`nucleus-title-${reviewCase.id}`} aria-busy={busy}>
    <header className="nucleus-header">
      <div><p className="review-kicker">OPERACIÓN UNIFICADA</p><h3 className="review-subtitle" id={`nucleus-title-${reviewCase.id}`}>Núcleo Resolutivo IA</h3><p className="review-muted">Una vista, una prioridad y las autoridades existentes trabajando como un único flujo.</p></div>
    ...
    <section ref={contextPanelRef} id={contextPanelId} role="tabpanel" aria-labelledby={`nucleus-context-tab-${activeContext}-${reviewCase.id}`} tabIndex={-1} className="nucleus-context-panel">
      {activeContext === "overview" ? <>
        <div className="nucleus-overview" aria-label="Resumen de zonas del workspace">{workspace.zones.filter((zone) => zone.id !== "summary").map((zone) => <article key={zone.id} className={`workspace-zone-card workspace-zone-${zone.state}`}><div className="review-row review-row-wrap"><h4>{zone.label}</h4><span className="review-badge">{zone.state}</span></div><p>{zone.safeSummary}</p><dl>{zone.metrics.map((entry) => <div key={entry.label} className={`workspace-metric-${entry.tone}`}><dt>{entry.label}</dt><dd>{entry.value}</dd></div>)}</dl><button type="button" className="review-button review-button-secondary" onClick={() => openWorkspaceSection(zone.id as NucleusSection, `Se abrió ${zone.label}; ninguna operación se ejecutó automáticamente.`)}>Abrir {zone.label}</button></article>)}</div>
      ...
      {activeContext === "workspace" ? <>
        <header className="nucleus-context-header"><div><p className="review-kicker">WORKSPACE RESOLUTIVO</p><h4>Operaciones y autoridades del caso</h4><p className="review-muted">Selecciona sólo la zona que necesitas revisar. El contenido técnico se carga bajo demanda.</p></div></header>
        <div className="nucleus-overview" aria-label="Zonas del workspace">{workspace.zones.map((zone) => <article key={zone.id} className={`workspace-zone-card workspace-zone-${zone.state}`}><div className="review-row review-row-wrap"><h4>{zone.label}</h4><span className="review-badge">{zone.state}</span></div><p>{zone.safeSummary}</p><dl>{zone.metrics.map((entry) => <div key={entry.label} className={`workspace-metric-${entry.tone}`}><dt>{entry.label}</dt><dd>{entry.value}</dd></div>)}</dl>{zone.id === "knowledge" ? <small>Experiencia relevante. La evidencia actual prevalece.</small> : null}{zone.id === "summary" ? <small>Vista derivada de snapshots existentes.</small> : <button type="button" className="review-button review-button-secondary" onClick={() => openWorkspaceSection(zone.id as NucleusSection)}>Abrir {zone.label}</button>}</article>)}</div>
        ...
        {activeSection ? <div ref={detailRef} id={`workspace-zone-${reviewCase.id}`} tabIndex={-1} className="nucleus-authority-detail" aria-label={`${sectionLabels[activeSection]} · detalle autoritativo`}><div className="review-row review-row-wrap"><div><p className="review-kicker">WORKSPACE CONTEXTUAL</p><h4>{sectionLabels[activeSection]}</h4><p className="review-muted">{getOperationalWorkspaceZone(workspace, activeSection).safeSummary}</p></div><button type="button" className="review-button review-button-secondary" onClick={() => setActiveSection(null)}>Cerrar sección</button></div><details className="nucleus-details"><summary>Timeline contextual ({workspace.contextualTimeline[activeSection].length})</summary>{workspace.contextualTimeline[activeSection].length ? <ol className="nucleus-timeline">{workspace.contextualTimeline[activeSection].map((event) => <li key={event.id}><strong>{event.label}</strong><span>{event.safeSummary}</span><small>{event.fingerprint}</small></li>)}</ol> : <p className="review-empty">Este caso todavía no tiene actividad resolutiva.</p>}</details><Suspense fallback={<FeedbackSkeleton label={`Cargando ${sectionLabels[activeSection]}`} />}><LazyOperationalWorkspaceSection zone={activeSection} reviewCase={reviewCase} canEdit={canEdit} /></Suspense></div> : null}
      </> : null}

      {activeContext === "timeline" ? <section className="nucleus-timeline-context" aria-labelledby={`nucleus-timeline-title-${reviewCase.id}`}><header className="nucleus-context-header"><div><p className="review-kicker">ACTIVIDAD DEL CASO</p><h4 id={`nucleus-timeline-title-${reviewCase.id}`}>Timeline resolutivo</h4><p className="review-muted">Eventos derivados de los snapshots y checkpoints existentes; no es un log paralelo.</p></div></header>{model.timeline.length ? <ol className="nucleus-timeline">{model.timeline.map((event) => <li key={event.id}><strong>{event.label}</strong><span>{event.safeSummary}</span><small>{event.occurredAt ? `${new Date(event.occurredAt).toLocaleString("es-ES")} · ` : ""}{event.fingerprint}</small></li>)}</ol> : <p className="review-empty">Este caso todavía no tiene actividad resolutiva.</p>}</section> : null}
    </section>
  </section>;
}
