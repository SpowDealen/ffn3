...
function reviewSignals(cases: readonly ReviewCase[]): OperatorSignal[] {
  return cases.flatMap((reviewCase) => {
    ...
    return [signal({
      ...
      source: "review",
      sourceLabel: "Centro de revisión",
      title: reviewCase.title,
      summary: blocked ? "Necesita una decisión o comprobación antes de continuar." : active ? "La revisión continúa en su pantalla de origen." : "Caso pendiente de decisión en el Centro de revisión.",
...
function notificationSignals(notifications: readonly LabNotification[]): OperatorSignal[] {
  return notifications.flatMap((notification) => {
    ...
    return [signal({
      ...
      summary: "Entrega pendiente; el registro permanece en el historial de actividad.",
      kind: "active",
      priority: "low",
      temporal: "current",
      actionable: false,
